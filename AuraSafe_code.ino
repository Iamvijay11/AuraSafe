...

  {
    ...
    if (Serial1.available())
    {
      ...
      if (inChar == '\n') {
        ...
        //check the state
        if (fromGSM == "v\r" || fromGSM == "V\r" || fromGSM == "Send Location\r")
        {
    ...
    if (digitalRead(SOS_Button) == LOW && CALL_END == 1)
    {
      ...
      if (c == 5)
      {
        ...
        // Transmit SOS alert via RF transmitter
        Send_RF_Transmission("SOS Alert");
      }
...
void Send_SMS(String message)
{
  for (int i = 0; i < 2; i++)
  {
    ...
    delay(1000);
    Serial1.println("AT+CMGS=\"" + SOS_NUM[i] + "\"\r");
    delay(1000);
...
void Send_RF_Transmission(String message) {
  Serial1.println(message); // Send data via A9G TX pin to RF transmitter
  Serial.print("RF Transmission: ");
  delay(5000);
